...
slot_to_entry() {
  case "$1" in
    A) printf 'betterframe-a.conf' ;;
    B) printf 'betterframe-b.conf' ;;
    *) exit 2 ;;
  esac
}
...
read_primary_from_mount() {
  local mountpoint="$1"
  awk '$1 == "default" { print $2; exit }' "${mountpoint}/loader/loader.conf" 2>/dev/null \
    | sed -e 's/^betterframe-//' -e 's/\.conf$//' | tr '[:lower:]' '[:upper:]'
}
...
write_primary_to_mount() {
  ...
  local primary="$2"
  local entry
  entry="$(slot_to_entry "$primary")"
  install -d -m 755 "${mountpoint}/loader"
  if [ -f "${mountpoint}/loader/loader.conf" ]; then
    if grep -q '^default ' "${mountpoint}/loader/loader.conf"; then
      sed -i "s/^default .*/default ${entry}/" "${mountpoint}/loader/loader.conf"
    else
      printf 'default %s\n' "$entry" >> "${mountpoint}/loader/loader.conf"
    fi
  else
    {
      printf 'default %s\n' "$entry"
  ...
  sync -f "${mountpoint}/loader/loader.conf" 2>/dev/null || sync
}
