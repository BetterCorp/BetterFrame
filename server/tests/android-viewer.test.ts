...
function fixture() {
  const kiosk = { id: "viewer", name: "Viewer", enabled: true, capabilities: ["android-viewer"], key_hash: "hash", operator_console_enabled: true, simple_vms_enabled: true, operator_tools_json: '[{"label":"secret","url":"https://secret"}]' };
...
test("Android bundle follows resolved assigned camera entities and preserves HTML/signage only", async () => {
  ...
  assert.equal(bundle.layouts[0]?.cells[0]?.smart_url, undefined);
