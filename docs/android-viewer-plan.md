...
- Scope signage storage initialization to the configured trusted top-level player origin; never inject tokens into redirected third-party pages or child frames. WebViews share same-origin browser storage. Allow matching assigned storage settings together; if another visible cell on the resolved origin has different settings, show a session-configuration placeholder that can be expanded individually. This keeps independent origins and compatible same-origin pages within the 32-view budget without overwriting another screen's session.
...
- Allow up to 32 visible camera streams in both camera-only and mixed camera/web layouts. Operators tune camera count, substream resolution, frame rate and bitrate for their hardware; the application ceiling does not guarantee that a device can decode all 32 streams reliably.
- Allow up to 32 visible web/HTML/signage cells alongside cameras, within the 64-cell layout limit. Qualify camera-only, mixed layouts and fullscreen signage on the chosen devices. Begin with lower-cost substreams and pages, then increase the workload from measurements. Count video inside WebView against the same device decoder budget as native camera streams.
