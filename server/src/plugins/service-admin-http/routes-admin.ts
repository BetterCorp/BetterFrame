...
import { legacyBackupUnavailable } from "../../shared/backup.js";
...
export function registerAdminRoutes(app: H3, deps: AdminDeps): void {
  ...
  // The retired SQLite archive cannot preserve the PostgreSQL database.
  // Reject before reading uploads or writing any key material.
  for (const path of ["/admin/backup/download", "/admin/backup/restore"]) {
    app.post(path, () => new Response(legacyBackupUnavailable, { status: 409 }));
  }
